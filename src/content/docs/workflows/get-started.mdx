...
TODO - explain each field and why they exist, what the naming conventions are, etc.
...
// Beta requirement: your Workers script must still export a default
// event handler. In this case, we'll export a fetch() handler.
export default {
	async fetch() {
...
4. Via the REST API or wrangler
